...
class DisKT(nn.Module):
    def __init__(self, num_c, emb_size=128, dropout=0.1, num_blocks=2, kq_same=0, 
        ...
        self.model = Architecture(num_skills=num_c, num_blocks=num_blocks, n_heads=num_attn_heads, dropout=dropout,
                                    embedding_size=embed_l, d_feature=embed_l // num_attn_heads, d_ff=d_ff,  kq_same=self.kq_same, seq_len=seq_len)

        self.ffn = FeedForward(d_model=embed_l, inner_size=embed_l*2, dropout=dropout)
        self.dual_attention = DualAttention(n_heads=num_attn_heads, d_model=embed_l)
        self.out = nn.Sequential(
            nn.Linear(embed_l + embed_l*2, final_fc_dim), 
            nn.ReLU(), 
    ...
    def forward(self, q, r, qshft=None, rshft=None, m=None, sm=None, at=None, pid_data=None):
        ...
        pos_q_embed_data, pos_qa_embed_data, _ = self.rasch_emb(masked_r * s_seq, masked_r * pid_seq, 2 - masked_r)
        neg_q_embed_data, neg_qa_embed_data, _ = self.rasch_emb((1-masked_r) * s_seq, (1-masked_r) * pid_seq, 2 * masked_r)
        q_embed_data, qa_embed_data, pid_embed_data = self.rasch_emb(s_seq, pid_seq, masked_r)
